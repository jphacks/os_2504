...
import { buildQrSource, generateMemberId, saveRoomMemberId } from '../../../utils/groupUtils'
...
const HomePage = (): ReactElement => {
  ...
  const [roomResponse, setRoomResponse] = useState<RoomCreateResponse | null>(null)
  ...
  const handleCreateRoom = async () => {
    ...
    const payload = {
      room_name: roomName.trim() || '新しいグループ',
      settings: {
        latitude: location?.latitude ?? 0,
        longitude: location?.longitude ?? 0,
        radius: radius / 1000, // Convert meters to kilometers
        min_price_level: priceRange[0],
        max_price_level: priceRange[1],
      },
    }
    ...
    try {
      ...
      if (!result.ok) {
        throw new Error(result.error?.message || 'Room creation failed')
      }

      setProgress(80)
      const roomData: RoomCreateResponse = result.data
      setRoomResponse(roomData)
      saveRoomMemberId(roomData.room_code, organizerId)

      // Create member for organizer
      await fetch(`/api/rooms/${roomData.room_code}/members`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          member_name: organizerName.trim() || '幹事',
        }),
      })
      setProgress(100)
    } catch (err) {
  ...
  const participantLink = useMemo(() => {
    ...
    return `${window.location.origin}/r/${roomResponse.room_code}`
  }, [roomResponse])

  const organizerRoute = useMemo(() => {
    if (!roomResponse || !memberId) {
      return ''
    }
    return `/r/${roomResponse.room_code}?memberId=${encodeURIComponent(memberId)}`
  }, [roomResponse, memberId])
                        ...
                        <Slider
                          ...
                          onChange={(_event, value) => {
                            if (Array.isArray(value) && value.length === 2) {
                              setPriceRange([value[0] ?? 0, value[1] ?? 4])
                            }
                          }}
                          min={0}
                          max={4}
                ...
                <CardContent>
                  <Stack spacing={2}>
                    ...
                    <Stack direction="row" alignItems="center" spacing={1}>
                      <Typography sx={{ flexGrow: 1, wordBreak: 'break-all', fontSize: '0.875rem' }}>
                        {participantLink}
                      </Typography>
                      <IconButton
