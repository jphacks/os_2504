...
## 🎥 デモ動画  
➡️ https://youtube.com/shorts/Kcumb8yQy6A?feature=share
...
🛠️ Task コマンドによるローカル開発チュートリアル

1. 前提ツール  
   - [go-task](https://taskfile.dev/installation/)（v3 以上）  
   - Docker / Docker Compose  
   - `.env` を `.env.example` などからコピーし必要な値を設定
...
3. 依存関係のセットアップ（コンテナ上で完結）  
   ```bash
   task deps:backend   # FastAPI 用イメージをビルド
   task deps:frontend  # frontend コンテナ内で npm install
   ```
...
困ったときは `task docker:logs` で `backend` と `mysql` のログをフォローできます。詳しい定義は `Taskfile.yml` を参照してください。
...
🔗 開発ドキュメントは `documents/README.md` を参照してください。
